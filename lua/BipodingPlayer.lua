Hooks:PostHook(PlayerBipod, "_check_action_steelsight", "BipopADS", function(self, t, input, ...)
...
--[[
    -- Prints the Factory id of the weapon currently equipped when shooting while in Bipod state
    -- why did i even make this part i alr knew it, dumdum

Hooks:PostHook(PlayerBipod, "_update_movement", "BipopSyb", function(self, t, dt, ...)
    
	local equipped = self._ext_inventory:equipped_unit():base()
    local equipped_unit_name = equipped._factory_id
        
        if self._shooting then
            log(equipped_unit_name)
        end
end)
--]]

Hooks:PostHook(PlayerBipod, "_update_movement", "BipopLoggg", function(self, t, dt, ...)
